...
struct segment_tree {
    ...
    int query_helper(int start, int end, int left, int right, int node) {
        ...
        if(start >= left && end <= right)
            return st[node];
        // case-3: partial overlap: visit the left and right children to find the complete overlap
        int mid = start + (end - start) / 2;
    ...
    // find min value in arr[left to right]
    int query(int left, int right) {
        // we will always strt from the root node
