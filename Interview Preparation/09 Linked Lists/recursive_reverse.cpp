...
node *recursive_reverse(node *head) {
    // base case, smallest list you need to reverse
    // list with 1 node or no node
    if(head->next == NULL or head == NULL)
        return head;
    // make a call on the smaller list
    ...
    node *temp = head->next;
    temp->next = head;
    head->next = NULL; // head node will be the last node in the reversed list
...
int main() {
    ...
    cout << "original list: " << head << endl;

    head = recursive_reverse(head);

    cout << "reversed list: " << head << endl;
