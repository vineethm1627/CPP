...
bool compare(pair<int, int> p1, pair<int, int> p2) {
	return p1.second < p2.second; // ascending order of end point of the circle diameter
}
...
int main() {
	...
	for(int i = 1; i < n; i++) {
		if(p[i].first < start) {
			// the given circle is intersecting
			count++;
		}
		else {
			// the given circle is not intersecting, update the start value
			start = p[i].second;
		}
	}
	cout << count << endl;
